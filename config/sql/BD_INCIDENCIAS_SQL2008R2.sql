...
BEGIN
    DROP DATABASE BD_INCIDENCIAS_2008;
END
GO

CREATE DATABASE BD_INCIDENCIAS_2008;
GO

USE BD_INCIDENCIAS_2008;
GO
...
CREATE TABLE USUARIO (
    USU_codigo SMALLINT IDENTITY(0,1) NOT NULL,
...
CREATE VIEW vw_usuario_detalle AS
SELECT 
    u.USU_nombre, 
    u.USU_codigo,
    u.USU_password, 
...
CREATE VIEW vw_incidencias_totales AS
...
CREATE VIEW vw_incidencias_pendientes AS
WITH IncidenciasOrdenadas AS (
    SELECT
        I.INC_numero,
        C.CIE_numero,
    ...
    LEFT JOIN USUARIO U ON U.USU_codigo = I.USU_codigo
    LEFT JOIN PERSONA P ON P.PER_codigo = U.PER_codigo
    WHERE (I.EST_codigo IN (3, 4) OR C.EST_codigo IN (3, 4)) and 
	(I.EST_codigo NOT IN (7) OR C.EST_codigo NOT IN (7))  -- Filtramos para no mostrar estado 7 (CERRADO)
)
-- Seleccionamos solo el registro más reciente (rn = 1)
SELECT 
    INC_numero,
...
FROM IncidenciasOrdenadas
WHERE rn = 1
  AND EST_codigo != 7;  -- Excluye los registros cuyo estado es "CERRADO"
GO

--Vista para listar las incidencias totales para el usuario en la consulta
CREATE VIEW vw_incidencias_totales_usuario AS
WITH IncidenciasOrdenadas AS (
    SELECT
        I.INC_numero,
        I.INC_numero_formato,
...
CREATE VIEW vw_incidencias_asignadas AS
SELECT 
  I.INC_numero,
  ASI.ASI_codigo,
  R.REC_numero,
...
CREATE VIEW vw_notificaciones_soporte AS
SELECT 
    I.INC_numero,
    I.INC_numero_formato,
    A.ASI_codigo,
...
CREATE VIEW vw_notificaciones_administrador AS
SELECT 
I.INC_numero,
(CONVERT(VARCHAR(10), I.INC_fecha, 103) + ' - ' + CONVERT(VARCHAR(5), I.INC_hora, 108)) AS fechaIncidenciaFormateada,
A.ARE_nombre,
...
CREATE VIEW vw_incidencias_mes_actual
AS
...
CREATE VIEW vw_incidencias_mes_actual_area
AS
  SELECT 
  ...
  FROM INCIDENCIA i
  INNER JOIN AREA a ON a.ARE_codigo = i.ARE_codigo
  WHERE i.INC_FECHA >= DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)
...
CREATE VIEW vw_pendientes_mes_actual
AS
...
CREATE VIEW vw_pendientes_mes_actual_area
AS
  SELECT 
    a.ARE_codigo AS area,
...
AS
  ...
  WHERE r.EST_codigo IN (4)
    AND r.ultimaFecha >= DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)
  GROUP BY a.ARE_codigo;
GO

-- VISTAS PARA CONTAR LOS CIERRES DEL MES ACTUAL - ADMINISTRADOR Y SOPORTE
CREATE VIEW vw_cierres_mes_actual
AS
  SELECT
  ...
  WHERE (I.EST_codigo = 7 OR C.EST_codigo = 7)
    AND I.INC_FECHA >= DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)
    AND I.INC_FECHA < DATEADD(DAY, 1, DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) + 1, 0))
    AND C.CIE_FECHA >= DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)
    AND C.CIE_FECHA < DATEADD(DAY, 1, DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) + 1, 0));
GO


-- Vista para contar los cierres del mes actual por area para el usuario
CREATE VIEW vw_cierres_mes_actual_area
AS
  SELECT
   a.ARE_codigo AS area,
   COUNT(*) AS cierres_mes_actual
...
CREATE VIEW vw_total_recepciones_mes_actual
AS
  SELECT 
    SUM(
...
CREATE VIEW vw_area_mas_incidencias_mes
AS
  SELECT TOP 1 
...
CREATE VIEW vw_reporte_incidencias_area_equipo AS
SELECT
    I.INC_numero,
	C.CIE_numero,
    I.INC_numero_formato,
...
CREATE VIEW vw_reporte_incidencias_equipos AS
...
CREATE VIEW vw_reporte_incidencias_area AS
...
CREATE VIEW vw_incidencias_recepcionadas AS
WITH UltimaRecepcion AS (
SELECT
  I.INC_numero,
  I.INC_numero_formato,
...
CREATE VIEW vw_reporte_pendientes_cierre AS
SELECT 
    I.INC_numero,
    INC_numero_formato,
    (CONVERT(VARCHAR(10), INC_fecha, 103)) AS fechaIncidenciaFormateada,
...
CREATE VIEW vw_area_mas_afectada AS
...
CREATE VIEW vw_equipos_mas_afectados AS
SELECT 
...
CREATE VIEW vw_eventos_totales AS
SELECT
  (
...
CREATE VIEW vw_auditoria_login AS
SELECT *
...
CREATE VIEW vw_auditoria_registrar_incidencia AS
SELECT  
...
CREATE VIEW vw_eventos_incidencias AS
SELECT
  (
    CONVERT ( VARCHAR ( 10 ), A.AUD_fecha, 103 ) + ' - ' + STUFF( RIGHT( '0' + CONVERT ( VARCHAR ( 7 ), A.AUD_hora, 0 ), 7 ), 6, 0, ' ' ) 
  ) AS fechaFormateada,
...
CREATE VIEW vw_eventos_recepciones AS
SELECT
  (
    CONVERT(VARCHAR(10), A.AUD_fecha, 103) + ' - ' + STUFF(RIGHT('0' + CONVERT(VARCHAR(7), A.AUD_hora, 0), 7), 6, 0, ' ')
  ) AS fechaFormateada,
...
CREATE VIEW vw_eventos_asignaciones AS
SELECT
  (
    CONVERT(VARCHAR(10), A.AUD_fecha, 103) + ' - ' + STUFF(RIGHT('0' + CONVERT(VARCHAR(7), A.AUD_hora, 0), 7), 6, 0, ' ')
  ) AS fechaFormateada,
...
CREATE VIEW vw_eventos_mantenimiento AS
SELECT
  (
    CONVERT(VARCHAR(10), A.AUD_fecha, 103) + ' - ' + STUFF(RIGHT('0' + CONVERT(VARCHAR(7), A.AUD_hora, 0), 7), 6, 0, ' ')
  ) AS fechaFormateada,
...
CREATE VIEW vw_eventos_cierres AS
SELECT
  (
    CONVERT(VARCHAR(10), A.AUD_fecha, 103) + ' - ' + STUFF(RIGHT('0' + CONVERT(VARCHAR(7), A.AUD_hora, 0), 7), 6, 0, ' ')
  ) AS fechaFormateada,
...
CREATE VIEW vw_eventos_personas AS
SELECT
  (
    CONVERT ( VARCHAR ( 10 ), A.AUD_fecha, 103 ) + ' - ' + STUFF( RIGHT( '0' + CONVERT ( VARCHAR ( 7 ), A.AUD_hora, 0 ), 7 ), 6, 0, ' ' ) 
  ) AS fechaFormateada,
...
CREATE VIEW vw_eventos_usuarios AS 
SELECT
  (CONVERT ( VARCHAR ( 10 ), A.AUD_fecha, 103 ) + ' - ' + STUFF( RIGHT( '0' + CONVERT ( VARCHAR ( 7 ), A.AUD_hora, 0 ), 7 ), 6, 0, ' ' ) 
  ) AS fechaFormateada,
  A.AUD_fecha,
...
CREATE VIEW vw_eventos_areas AS
SELECT
  (
...
CREATE VIEW vw_eventos_bienes AS
SELECT
  (
    CONVERT (VARCHAR(10), A.AUD_fecha, 103) + ' - ' + 
    STUFF(RIGHT('0' + CONVERT (VARCHAR(7), A.AUD_hora, 0), 7), 6, 0, ' ')
...
CREATE VIEW vw_eventos_soluciones AS
SELECT
  (
    CONVERT (VARCHAR(10), A.AUD_fecha, 103) + ' - ' + 
    STUFF(RIGHT('0' + CONVERT (VARCHAR(7), A.AUD_hora, 0), 7), 6, 0, ' ')
...
CREATE FUNCTION dbo.fn_tiempo_mantenimiento (
    @ASI_fecha DATE, 
    @ASI_hora TIME(0), 
    @MAN_fecha DATE, 
    @MAN_hora TIME(0)
...
CREATE PROCEDURE sp_crear_usuario_inicial
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Hashear la contraseña usando HASHBYTES con SHA1 (20 bytes)
    DECLARE @hashed_password VARBINARY(20);  -- Cambiar a 20 bytes para SHA1
    ...
    -- Aplicar el hash usando SHA1 (disponible en SQL Server 2008 R2)
    SET @hashed_password = HASHBYTES('SHA1', @to_hash);

    -- Verificar si el hash es nulo
    IF @hashed_password IS NULL
    BEGIN
        RAISERROR('Error al generar el hash de la contraseña.', 16, 1);
        RETURN;
    END

    -- Insertar el usuario inicial
    INSERT INTO USUARIO (USU_nombre, USU_password, USU_salt, PER_codigo, ROL_codigo, ARE_codigo, EST_codigo, USU_fecha_creacion)
    VALUES (@USU_nombre, @hashed_password, @salt, NULL, @ROL_codigo, @ARE_codigo, 1, GETDATE());
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM USUARIO WHERE USU_nombre = @USU_nombre)
    BEGIN
        RAISERROR('El nombre de usuario ya está registrado. Por favor elija otro nombre de usuario.', 16, 1);
        RETURN;
    END
    ...
    -- Convertir la contraseña en bytes y concatenar con el salt
    DECLARE @password_bytes VARBINARY(100) = CONVERT(VARBINARY(100), @USU_password);
    DECLARE @salt_bytes VARBINARY(16) = CAST(@salt AS VARBINARY(16));
    DECLARE @to_hash VARBINARY(116) = @password_bytes + @salt_bytes;

    -- Hashear la contraseña usando SHA1 (disponible en SQL Server 2008 R2)
    DECLARE @hashed_password VARBINARY(20);  -- Cambio a VARBINARY(20) para SHA1
    SET @hashed_password = HASHBYTES('SHA1', @to_hash);
...
BEGIN
    ...
    DECLARE @stored_password VARBINARY(20);  -- Ajuste a 20 bytes para SHA1
...
CREATE PROCEDURE sp_editar_bien
...
CREATE PROCEDURE sp_editar_solucion
...
CREATE PROCEDURE sp_habilitar_incidencia
...
CREATE PROCEDURE sp_deshabilitar_incidencia
	@codigoIncidencia SMALLINT
AS
BEGIN
...
CREATE PROCEDURE sp_registrar_solucion
...
CREATE PROCEDURE sp_habilitar_solucion
...
CREATE PROCEDURE sp_deshabilitar_solucion
	@codigoSolucion SMALLINT
AS
BEGIN
...
BEGIN
    ...
    BEGIN CATCH 
        ...
        SELECT 
            ...
            @ErrorSeverity = ERROR_SEVERITY(),
            @ErrorState = ERROR_STATE();
        -- Usar RAISERROR para lanzar el error
        RAISERROR(@ErrorMessage, @ErrorSeverity, @ErrorState);
    END CATCH
END;
GO
...
CREATE PROCEDURE sp_encolar_incidencia
    @ASI_codigo SMALLINT
AS 
BEGIN
    SET NOCOUNT ON;
...
CREATE PROCEDURE sp_insertar_cierre
  @CIE_fecha DATE,
  @CIE_hora TIME,
...
CREATE PROCEDURE sp_eliminar_cierre
    @IdCierre INT
AS
BEGIN
    BEGIN TRANSACTION;
...
CREATE PROCEDURE sp_consultar_incidencias_totales
  @area INT = NULL,
...
CREATE PROCEDURE sp_filtrar_incidencias_totales_fecha
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
BEGIN
...
CREATE PROCEDURE sp_consultar_incidencias_pendientes
  @area INT,
  @estado INT,
  @fechaInicio DATE,
  @fechaFin DATE
...
CREATE PROCEDURE sp_consultar_incidencias_asignadas
  @usuario INT = NULL,
  @codigoPatrimonial CHAR(12) = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
...
CREATE PROCEDURE sp_filtrar_incidencias_asignadas
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_incidencias_cerradas
  @area INT = NULL,
  @codigoPatrimonial CHAR(12) = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
...
CREATE PROCEDURE sp_filtrar_incidencias_cerradas
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_incidencias_usuario
  @area INT = NULL,
  @codigoPatrimonial CHAR(12) = NULL,
  @estado INT = NULL,
  @fechaInicio DATE = NULL,
...
CREATE PROCEDURE sp_filtrar_incidencias_area
  @area INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_filtrar_incidencias_equipo
  @codigoPatrimonial CHAR(12) = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_filtrar_areas_afectadas
  @categoria INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_filtrar_equipos_afectados
  @codigoPatrimonial CHAR(12) = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_totales
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_login
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
-- CREATE PROCEDURE sp_consultar_auditoria_registro_incidencia
--     @fechaInicio DATE = NULL,
--     @fechaFin DATE = NULL
-- AS
-- BEGIN 
...
CREATE PROCEDURE sp_consultar_eventos_incidencias
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_recepciones
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_asignaciones
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_mantenimiento
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_cierres
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_usuarios
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_personas
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_areas
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_bienes
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_categorias
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
CREATE PROCEDURE sp_consultar_eventos_soluciones
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
